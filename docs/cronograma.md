...
| **02** | Estudos Técnicos | **DSP:** Estudo da biblioteca `driver/i2s.h` e FFT. **Firmware:** Configuração do simulador Wokwi. **Conexão:** Estudo da API de Bots do Telegram. **UX:** Pesquisa de padrões visuais de alerta. |
| **03** | Validação Digital | **DSP:** Coleta de áudios de campainhas para análise. **Firmware:** Hello World de LED no simulador. **Conexão:** Teste de envio de mensagens simples via Bot. **UX:** Desenho do fluxo de interação usuário-dispositivo. |
| **04** | Modelagem Inicial | **DSP:** Análise espectral dos áudios no Python. **Firmware:** Teste de concorrência Wi-Fi no simulador. **UX:** Esboços iniciais da case considerando acústica para o microfone INMP441. |
